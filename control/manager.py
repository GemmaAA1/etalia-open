...
def load(file):
    ...
    call([os.path.join(root_path, "manage.py"), "loaddata",
          os.path.join(root_path, "control", file)])
...
def dump_data():
    file = os.path.join(root_path, "control", INIT_DEFAULT_FIXTURE_FILE)
...
def init_models():
    ...
    with open(os.path.join(root_path, 'control', MODEL_FILE)) as stream:
    ...
    if not Model.objects.filter(name=model_params['name']).exists():
        ...
        print('NLP Model already initialized!')
...
if __name__ == '__main__':
    ...
    from control.utils.avatar import AvatarGenerator
