...
from ..utils import StreamScoring, SimpleAverage, ThresholdAverage, \
...
class ScoringTest(UserFeedTestCase):
    ...
    def test_scoring_can_be_instantiated(self):
        StreamScoring(model=self.model, user=self.user)

    def test_scoring_canNOT_be_run_if_not_ready(self):
        scoring = StreamScoring(model=self.model, user=self.user)
